...
@section('content')
<div class="max-w-2xl">
    <form action="{{ route('admin.permissions.store') }}" method="POST" class="space-y-xl">
        @csrf
        ...
        <div class="bg-surface-container-high border border-outline-variant/20 rounded-lg overflow-hidden">
            ...
            <div class="p-lg space-y-lg">
                <div>
                    <label class="block font-label-caps text-label-caps text-on-surface-variant mb-xs" for="name">Nombre del Permiso *</label>
                    <input type="text" name="name" id="name" value="{{ old('name') }}"
                           ...
                           placeholder="Ej: create_posts" required>
                    @error('name')
                ...
                <div>
                    <label class="block font-label-caps text-label-caps text-on-surface-variant mb-xs" for="guard_name">Guard Name</label>
        ...
        <div class="flex items-center justify-end gap-md">
            <a href="{{ route('admin.permissions.index') }}" class="bg-surface-container-high border border-outline-variant/30 text-on-surface-variant font-label-caps text-label-caps px-lg py-sm rounded hover:bg-surface-container transition-colors active:scale-95">
                Cancelar
            </a>
